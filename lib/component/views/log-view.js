...
export class LogViewWindow {
    constructor(context, theme, logMaxLines = 100) {
    ...
    createWindowOnce() {
        ...
        const params = LayoutParams.$new(width, height, dp(this.context, 20), dp(this.context, 60), 2038, LayoutParams.FLAG_NOT_FOCUSABLE.value |
    ...
    bindDragForHeader() {
        if (!this.titleDragHandle || !this.windowRoot || !this.windowParams)
            return;
        ...
        let startX = 0;
        let startY = 0;
        this.titleDragHandle.setOnTouchListener(Java.registerClass({
            ...
            methods: {
                onTouch: (_v, event) => {
                    ...
                    if (action === MotionEvent.ACTION_MOVE.value) {
                        ...
                        Java.scheduleOnMainThread(() => {
                            try {
                                this.windowManager.updateViewLayout(this.windowRoot, this.windowParams);
                            }
                            catch { }
                        });
                        return true;
